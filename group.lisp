...
(defun unroll-meta-group (g)
  (let ((sym (car g))
        (num (cadr g))
        (list nil))
    (dotimes (i num)
      (setf list (cons (mk-signal-sym sym i) list)))
    list))

(defun unroll-meta-groups (groups)
  (mapcan #'(lambda (g)
              (if (symbolp g) (list g) (unroll-meta-group g)))
          groups))

(defmacro def-global-groups (&rest groups)
  (let ((list nil))
    (dolist (g (unroll-meta-groups groups))
      (dotimes (i 32)
        (setf list (cons `(defparameter ,(mk-signal-sym g i) (a-booleanv)) list))))
    `(progn ,@list)))

(defmacro list-global-groups (&rest groups)
  (let ((list nil))
    (dolist (g (reverse groups))
      (dotimes (i 32)
        (setf list (cons (mk-signal-sym g i) list))))
    `(list ,@list)))
